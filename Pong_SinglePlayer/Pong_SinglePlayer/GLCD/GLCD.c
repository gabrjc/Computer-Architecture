...
void PutChar_SIZED( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor, uint16_t SIZE )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
...
void GUI_Text_SIZED(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor, uint16_t SIZE)
{
    uint8_t TempChar;
    do
    {
        TempChar = *str++;  
        PutChar_SIZED( Xpos, Ypos, TempChar, Color, bkColor, SIZE);    
        if( Xpos < MAX_X - SIZE*8 )
        {
            Xpos += SIZE*8;
        } 
        else if ( Ypos < MAX_Y - SIZE*16 )
        {
            Xpos = 0;
            Ypos += SIZE*16;
        }   
